...
byte time_server[] = { 192, 168, 1, 7 }; // time.nist.gov

Client client(time_server, HUDSON_PORT); // <label id="code.ethernet.client"/>
...
void loop() {
  ...
  if (!client.connect()) { // <label id="code.ethernet.connect"/>
    ...
    delay(1000);
//    client.println("GET /job/arduino/api/json HTTP/1.0");
